    ...
    console.log (`Заморозков было: ${amount}`);

    //самые длинные заморозкки
    
    let period = 0;
    let periodZero = 0;
    for ( let j=0; j<winterDays.length-1; j++) {
        
        if (winterDays[j] < 0 ) {
            periodZero ++;
            if (period < periodZero) {
                period=periodZero;
    ...
    console.log (`Самый длинный период заморозков: ${period}`)
